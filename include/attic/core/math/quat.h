...
struct Quat {
    ...
    Quat normalized() const {
        float len = length();
        return len > 0.0f ? (*this) * (1.0f / len) : Quat();
    }
    Quat& normalize() {
        float len = length();
        if (len > 0.0f) {
    ...
    Mat3 to_mat3() const {
        ...
        return Mat3(1 - 2 * (yy + zz), 2 * (xy - wz), 2 * (xz + wy), 2 * (xy + wz),
    ...
    Quat slerp(const Quat& other, float t) const {
        ...
        if (dot_product > 0.9995f)
            return Quat(x + (target.x - x) * t, y + (target.y - y) * t, z + (target.z - z) * t,
                        w + (target.w - w) * t)
                .normalized();
        ...
        return Quat(x * s0 + target.x * s1, y * s0 + target.y * s1, z * s0 + target.z * s1,
                    w * s0 + target.w * s1);
    }
