...
struct Vec3 {
    ...
    Vec3 project(const Vec3& onto) const { return onto * (dot(onto) / onto.length_squared()); }
    ...
    Vec3 clamp(float min_val, float max_val) const {
        return Vec3(std::fmax(min_val, std::fmax(x, max_val)),
                    std::fmax(min_val, std::fmax(y, max_val)),
                    std::fmax(min_val, std::fmax(z, max_val)));
    }
